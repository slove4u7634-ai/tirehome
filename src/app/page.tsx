      ...
      <div className="mx-auto w-full max-w-[1600px] px-4 md:px-8">
        <div className="mt-6 md:mt-10 pb-16 md:pb-0 flex flex-col gap-8 md:gap-12">
          ...
          <section className="py-8 md:py-12 bg-white overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:overflow-x-auto gap-3 md:gap-4 md:pb-4 scrollbar-hide">
              {events.map((event) => (
                <Link href={`/events/${event.id}`} key={event.id} className="block shrink-0 transition-transform hover:-translate-y-1">
                  <img src={event.image} alt={event.title} className="w-full md:w-[400px] h-[90px] md:h-[150px] object-cover rounded-xl shadow-md border border-gray-100" />
                </Link>
              ))}
            </div>
          </section>
